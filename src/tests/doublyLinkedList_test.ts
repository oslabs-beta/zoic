...
describe("Doubly linked list tests", () => {
  ...
  it("Should properly add a sinlge node", () => {
    list.addHead({headers: {}, body: 'test1', status: 200}, 'C', 50, time);
  ...
  it("Should properly add nodes to the start of the linked list", () => {
    list.addHead({headers: {}, body: 'test2', status: 200}, 'B', 100, time);
    list.addHead({headers: {}, body: 'test3', status: 200}, 'A', 200, time);
  ...
  it("Should store properties corretly on nodes", () => {
    assertEquals(list.head?.value.body, 'test3');
    assertEquals(list.head?.next?.value.body, 'test2');
    assertEquals(list.head?.next?.next?.value.body, 'test1');
